...
st.set_page_config(page_title="투자 리스크 계산기", page_icon="📉")
st.title("📉 투자 리스크 계산기")

# 총 자산 불러오기
total_asset = get_total_asset()
st.sidebar.subheader("💰 총 자산: {:,} 원".format(total_asset))
...
# 손절 가격 계산
risk_result = calculate_stop_loss_price(total_asset, selected_position["positionAmt"], leverage, risk_ratio, entry_price)
...
st.write(f"최종 손절 가격: {stop_price:.2f}원")

# 부분 익절 처리
with st.expander("익절 처리"):
    profit = st.number_input("익절 금액", value=0)
    if profit > 0:
        percent_closed = st.slider("청산 비율 (%)", 1, 100, 50)
        realized = profit * (percent_closed / 100)
        new_asset = update_total_asset(realized)
        st.success(f"자산이 {int(realized):,}원 증가하여 총 자산은 {new_asset:,}원이 되었습니다.")
        trailing_stop = st.number_input("추적 손절 가격 (선택)", value=stop_price)
